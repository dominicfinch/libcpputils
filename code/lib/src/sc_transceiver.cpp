...
bool iar::utils::SCTransceiver::EncryptEncodeContent(const std::string& contact, const std::vector<uint8_t>& input, std::vector<uint8_t>& output)
{
    ...
    if(contact_it != _contacts.end())
    {
        ...
        std::vector<uint8_t> ciphertext, ciphertext_unencoded;
        ...
        if(_self.eccKP().encrypt(input, ciphertext_unencoded, iv, tag_unencoded))
        {
            if(Base64::encode(ciphertext_unencoded, ciphertext) &&
