...
public:
    explicit StreamBroadcaster(const std::string& stream_id)
        : _stream_id(stream_id) {}

    // gRPC subscription
    void add_grpc_subscriber(
    ...
    void broadcast_h264_frame(const uint8_t* data,
                              size_t size,
                              bool keyframe,
                              uint64_t ts_ms)
    {
...
private:
    ...
    std::vector<std::shared_ptr<
        grpc::ServerWriter<iar::rpc::StreamChunk>>> _grpc_subscribers;
