...
void addDirectedEdge(cpp::utils::AdjacencyList<Node, Cost>& g, Node a, Node b, Cost w)
{
    g[a].push_back({b, w});
}

template <typename Node, typename Cost>
void addUndirectedEdge(cpp::utils::AdjacencyList<Node, Cost>& g, Node a, Node b, Cost w)
{
    g[a].push_back({b, w});
    g[b].push_back({a, w});
}
...
bool test_astar_basic()
{
    ...
    cpp::utils::AStar<int, int> a(g, heuristic);
...
bool test_bellman_ford_negative_cycle()
{
    ...
    cpp::utils::BellmanFord<int, int> bf(g);
